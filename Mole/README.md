...
mo analyze                   ## Analyze (interactive disk explorer)
- `mo analyze` for overview mode
- `mo analyze /path/to/dir` to inspect a specific directory
- `mo analyze --projects` (requires `MO_FEATURE_PROJECTS=1`) to generate `.mole/projects.json` with project metadata (path, kind, pins, size, mtime, fingerprint)

### Analyze TUI quick keys
...
- Session is auto-restored on the next run (expires after 24h). Session is saved on quit and when navigating directories.
...
- Shared Data Paths:
  ...
  - **Index**: `~/.orbit/index.json`
- **Orbit Integration**: Mole now hydrates its overview mode from the shared Orbit index for instant startup.

## Status (system metrics dashboard)
- `mo status` for live CPU/GPU/memory/network/battery view
...
- **Environment Variables**:
  - `MO_DEBUG=1` for verbose logging
  - `MO_NO_COLOR=1` to disable colors
  - `MO_LANG=<en|zh|...>` set UI language where supported
  - `MO_FEATURE_PROJECTS=1` enables project discovery export for `mo analyze --projects`
