...
var paletteActions = []string{"Export", "Delete", "Pin", "Unpin"}
...
// selectedEntriesForAction returns the current multi-selection (large files or entries).
func (m model) selectedEntriesForAction() []dirEntry {
	...
	if len(out) == 0 && len(m.largeMultiSelected) == 0 && len(m.multiSelected) == 0 && !m.showLargeFiles && m.selected < len(m.entries) {
		// Fallback to single selection in list mode
		out = append(out, m.entries[m.selected])
		seen[m.entries[m.selected].Path] = true
	}
	if len(m.largeMultiSelected) > 0 {
		for path := range m.largeMultiSelected {
			for _, f := range m.largeFiles {
				if f.Path == path && !seen[path] {
					out = append(out, dirEntry{Name: f.Name, Path: f.Path, Size: f.Size, IsDir: false})
					seen[path] = true
					break
				}
			}
	...
	if m.showLargeFiles && len(out) == 0 && m.largeSelected < len(m.largeFiles) {
...
// batchPin pins or unpins current selection.
func (m *model) batchPin(add bool) {
	if !featureProjectsEnabled() {
		return
	}
	focus := loadFocus()
	selection := m.selectedEntriesForAction()
	seen := make(map[string]bool)
	for _, e := range selection {
		rel, err := filepath.Rel(m.path, e.Path)
		if err != nil {
			continue
		}
		if seen[rel] {
...
// beginDeleteFromSelection triggers delete confirmation using current selection.
func (m model) beginDeleteFromSelection() (tea.Model, tea.Cmd) {
	if len(m.multiSelected) == 0 && len(m.largeMultiSelected) == 0 {
		// Fall back to selected item
		if m.showLargeFiles {
			if m.largeSelected < len(m.largeFiles) {
				selected := m.largeFiles[m.largeSelected]
				m.deleteTarget = &dirEntry{Name: selected.Name, Path: selected.Path, Size: selected.Size}
			}
	...
	m.status = "Confirm delete (delete/backspace) or esc/q to cancel"
	return m, nil
}
...
func main() {
	...
	for _, arg := range os.Args[1:] {
		...
		if strings.HasPrefix(arg, "-") {
			// Unknown flag; ignore for now to preserve existing behavior
			continue
		}
	...
	if projectsMode {
		if !featureProjectsEnabled() {
			fmt.Fprintln(os.Stderr, "--projects requires MO_FEATURE_PROJECTS=1")
			os.Exit(1)
		}
		root := target
		if root == "" {
		...
		if root == "" {
			fmt.Fprintln(os.Stderr, "no root provided for --projects and HOME is empty")
			os.Exit(1)
		}
		if err := runProjectDiscovery(root); err != nil {
			fmt.Fprintf(os.Stderr, "project discovery failed: %v\n", err)
			os.Exit(1)
		}
		fmt.Printf("Project index saved to %s\n", filepath.Join(root, ".mole", "projects.json"))
		return
	}
	...
	p := tea.NewProgram(initialModel(abs, isOverview), tea.WithAltScreen())
...
func featureProjectsEnabled() bool {
	v := strings.ToLower(os.Getenv("MO_FEATURE_PROJECTS"))
	return v == "1" || v == "true" || v == "yes" || v == "on"
}

func calculateTotalSize(entries []dirEntry) int64 {
	var total int64
...
// initialModel builds the starting UI model (overview or directory) and hydrates from Orbit index when available.
func initialModel(path string, isOverview bool) tea.Model {
	// Apply high-contrast palette if requested
	initialHighContrast := strings.EqualFold(os.Getenv("MO_HIGH_CONTRAST"), "1") || strings.EqualFold(os.Getenv("MO_HIGH_CONTRAST"), "true")
	...
	var session *Session
	if featureProjectsEnabled() {
		if loadedSession, err := loadSession(); err == nil {
			session = loadedSession
		}
	}

	// Load directory data (non-overview)
	if !isOverview {
		if data, err := loadCacheFromDisk(path); err == nil {
			entries = data.Entries
			largeFiles = data.LargeFiles
			totalSize = calculateTotalSize(entries)
		} else {
			// Fallback to scanning
			result, _ := scanPathConcurrent(path, &filesScanned, &dirsScanned, &bytesScanned, &currentPath)
			entries = result.Entries
			largeFiles = result.LargeFiles
			totalSize = calculateTotalSize(entries)
			_ = saveCacheToDisk(path, scanResult{Entries: entries, LargeFiles: largeFiles})
		}
	}
	...
	m := model{
		...
		overviewFilesScanned: new(int64),
		overviewDirsScanned:  new(int64),
		overviewBytesScanned: new(int64),
		overviewCurrentPath:  new(string),
	}

	// In overview mode, create shortcut entries (prefer Orbit index)
	if isOverview {
		m.isOverview = true
		m.scanning = false
		if entries, err := loadOrbitIndexForPath(path); err == nil && len(entries) > 0 {
			m.entries = entries
			for _, e := range entries {
				if e.Size >= 0 {
					m.overviewSizeCache[e.Path] = e.Size
				}
			}
			m.totalSize = sumKnownEntrySizes(m.entries)
			m.status = "Ready (Orbit index)"
		} else {
		...
		} else if m.status != "Ready (Orbit index)" {
			m.status = "Ready"
		}
	} else {
		// Apply session if available (only for non-overview mode)
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case deleteProgressMsg:
		if msg.done {
			...
			if msg.err != nil {
				...
				// capture undo entry if provided
				if msg.undo != nil {
					m.lastUndo = msg.undo
				}
				if msg.path != "" {
				...
				if msg.undo != nil {
					m.status = fmt.Sprintf("Moved %d item(s) to trash. Ctrl+Z to undo.", msg.count)
				} else {
	...
	case undoResultMsg:
		...
		m.status = fmt.Sprintf("Restored %d item(s)", msg.restored)
...
func (m model) updateKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	// Palette interaction takes precedence
	if m.paletteActive {
		switch msg.String() {
		case "up", "k":
			if m.paletteIndex > 0 {
		...
		case "down", "j":
			...
			return m, nil
		case "enter":
			choice := paletteActions[m.paletteIndex]
			m.paletteActive = false
			switch choice {
			case "Export":
				m.exportSelection = m.selectedEntriesForAction()
				m.showExportModal()
				return m, nil
			case "Delete":
				return m.beginDeleteFromSelection()
			case "Pin":
				m.batchPin(true)
				m.status = "Pinned selection"
				return m, nil
			case "Unpin":
				m.batchPin(false)
				m.status = "Unpinned selection"
				return m, nil
			}
		case "esc", "q":
			m.paletteActive = false
			m.status = "Batch palette cancelled"
			return m, nil
		default:
			return m, nil
		}
	...
	if m.deleteConfirm {
		...
		case "delete", "backspace":
			...
			if len(pathsToDelete) == 1 {
				...
				return m, tea.Batch(moveToTrashCmd([]string{targetPath}, m.deleteCount), tickCmd())
			}

			m.status = fmt.Sprintf("Deleting %d items...", len(pathsToDelete))
			return m, tea.Batch(moveToTrashCmd(pathsToDelete, m.deleteCount), tickCmd())
		case "esc", "q":
	...
	// Handle export modal keys
	if m.exportModal.active {
		return m.handleExportModalKey(msg.String())
	}
	// Handle search input
	if m.searchMode {
		switch msg.Type {
	...
	case "q", "ctrl+c":
		...
		if err := m.saveSession(); err != nil {
			m.status = fmt.Sprintf("Failed to save session: %v", err)
		}
		return m, tea.Quit
	case "?":
		// Toggle help overlay (optional for now)
		return m, nil
	case "/":
		// Enter search mode
		if !m.searchMode {
		...
		return m, nil
	case "p":
		// Toggle pin for selected item if projects mode is enabled
	...
	case "e":
		...
		return m, nil
	case "d":
		// Toggle duplicates mode
		if featureProjectsEnabled() {
			m.duplicatesMode = !m.duplicatesMode
			m.searchMode = false
			m.searchInput = ""
			m.searchQuery = ""
		}
	...
	case "ctrl+z":
		...
		m.status = "Restoring..."
		return m, tea.Batch(undoCmd(m.lastUndo), tickCmd())
	case "b", "left", "h":
		if m.showLargeFiles {
			m.showLargeFiles = false
			return m, nil
		...
		if len(m.history) == 0 {
			...
			if !m.inOverviewMode() {
				// Save session before returning to overview
				if err := m.saveSession(); err != nil {
					m.status = fmt.Sprintf("Failed to save session: %v", err)
				}
	...
	case "C":
		...
		setHighContrast(m.highContrast)
		if m.highContrast {
			m.status = "High-contrast mode on"
		} else {
			m.status = "High-contrast mode off"
		}
		return m, nil
	case " ":
