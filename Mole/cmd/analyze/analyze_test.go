...
func TestInitialModelUsesOrbitIndexInOverview(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	orbitDir := filepath.Join(home, ".orbit")
	...
	idx := orbitIndex{
		...
		Projects: []orbitProject{
			{Path: "projA", SizeBytes: ptr(100)},
			{Path: filepath.Join(root, "projB"), SizeBytes: ptr(200)},
		},
	}
	...
	if err := os.WriteFile(filepath.Join(orbitDir, "index.json"), data, 0644); err != nil {
	...
	m := initialModel(root, true).(model)
	if !m.isOverview {
		t.Fatalf("expected overview mode")
	}

	if len(m.entries) != 2 {
		t.Fatalf("expected 2 entries from orbit index, got %d", len(m.entries))
	}

	wantSizes := map[string]int64{
		filepath.Join(root, "projA"): 100,
		filepath.Join(root, "projB"): 200,
	}

	for _, e := range m.entries {
		if size, ok := wantSizes[e.Path]; ok {
			if e.Size != size {
				t.Fatalf("size mismatch for %s: want %d, got %d", e.Path, size, e.Size)
			}
			if cached, ok := m.overviewSizeCache[e.Path]; !ok || cached != size {
				t.Fatalf("expected overview cache for %s to be %d, got %d", e.Path, size, cached)
			}
		} else {
	...
	if m.totalSize != int64(300) {
		t.Fatalf("unexpected total size; want 300, got %d", m.totalSize)
	}

	if m.status != "Ready (Orbit index)" {
		t.Fatalf("unexpected status; want 'Ready (Orbit index)', got %q", m.status)
	}
}
